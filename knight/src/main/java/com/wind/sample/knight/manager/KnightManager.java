package com.wind.sample.knight.manager;


import com.wind.sample.domain.Knight;
import com.wind.sample.knight.dao.KnightDao;
import com.wind.sample.knight.form.KnightForm;
import com.wind.sample.knight.form.KnightStatus;
    ...
    public boolean validate(KnightForm knightForm, Errors errors) {
        ...
        if (StringUtils.isEmpty(knightForm.getKnightId())) {
            if (KnightStatus.Dead.getId().equals(knightForm.getStatusId())) {
                errors.rejectValue("statusId", "why.do.you.choose.dead");
            }
        }
